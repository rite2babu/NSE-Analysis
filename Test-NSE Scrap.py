...
def get_adjusted_headers():
    return {
            ...
            "Accept": "*/*",
            "Postman-Token": "a3f6d8df-0608-4f59-aa03-9c57c41a24db",
...
def fetch_cookies():
    ...
    response = session.get('https://www.nseindia.com/', headers=get_adjusted_headers())
    print("Python Request Headers:")
    for k, v in response.request.headers.items():
        print(f"  {k}: {v}")
        
            
    response = requests.get(BASE_URL, headers=get_adjusted_headers())
    print("\nPython Response Headers:")
    for k, v in response.headers.items():
        print(f"  {k}: {v}")
    print("\nCookies received by Python:")
    ...
    if response.status_code != requests.codes.ok:
        ...
        logging.error( response)
        raise ValueError("Please try again in a minute -- fetching")
    logging.info(" fetch_cookies :")
    print( len(response.cookies.get_dict()))
    # dict asr = {'A': 'A'}
    tdict  = response.cookies.get_dict()
    tdict['nseappid']= "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJhcGkubnNlIiwiYXVkIjoiYXBpLm5zZSIsImlhdCI6MTc1MTgzMzE2NiwiZXhwIjoxNzUxODQwMzY2fQ.vXS40E2iv5n4eWiX_nHHykAW1T0MH0E7edvv57jxPx0"
    tdict['nsit']= "gmeiz_2Fsgaj8j_01hcDk6p_"
    return tdict
...
def scrape_data(start_date, end_date, name=None, input_type='stock'):
    ...
    # cookies["bm_sv"]="CACA693DC8CB1FCB5293C84FE0B7EA54~~1"
...
def main() :
    ...
    Get_Stock_Data3(['ITC'], 10, MAX_THREADS=1)
