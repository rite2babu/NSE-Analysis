"""
Apply Plotly horizontal bar charts to NSE_Analysis.ipynb (markdown format)
Replaces matplotlib sections 4d and 4e with improved Plotly versions
"""

# Read the markdown notebook
with open('NSE_Analysis.ipynb', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# New Section 4d - Plotly Gainers Chart
new_4d = '''## 4d. Top Gainers Chart (Plotly - Horizontal Bars)
...
# New Section 4e - Plotly Losers Chart
new_4e = '''## 4e. Top Losers Chart (Plotly - Horizontal Bars)
...
# Find section 4d (starts at line 604)
section_4d_start = None
section_4d_end = None
for i, line in enumerate(lines):
    if '## 4d. Top Gainers Chart (Grouped Bars)' in line:
        section_4d_start = i
    elif section_4d_start is not None and line.startswith('## 4e'):
        section_4d_end = i
        break

# Find section 4e (starts at line 655)
section_4e_start = None
section_4e_end = None
for i, line in enumerate(lines):
    if '## 4e. Top Losers Chart (Grouped Bars)' in line:
        section_4e_start = i
    elif section_4e_start is not None and line.startswith('## 4f'):
        section_4e_end = i
        break

modified = 0

if section_4d_start is not None and section_4d_end is not None:
    print(f"Replacing Section 4d (lines {section_4d_start+1}-{section_4d_end})...")
    lines[section_4d_start:section_4d_end] = [new_4d + '\n']
    modified += 1
    
    # Adjust section_4e indices after modification
    line_diff = section_4d_end - section_4d_start - 1
    if section_4e_start is not None:
        section_4e_start -= line_diff
    if section_4e_end is not None:
        section_4e_end -= line_diff

if section_4e_start is not None and section_4e_end is not None:
    print(f"Replacing Section 4e (lines {section_4e_start+1}-{section_4e_end})...")
    lines[section_4e_start:section_4e_end] = [new_4e + '\n']
    modified += 1
